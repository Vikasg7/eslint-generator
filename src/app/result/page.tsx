...
function ResultPageContent() {
  ...
  const answers = useMemo(() => decodeAnswers(id), [id])
  ...
  const tags = useMemo(
    () =>
      [
        answers.framework
          ? (LABEL_MAP[answers.framework] ?? answers.framework)
          : "",
        answers.lang ? (LABEL_MAP[answers.lang] ?? answers.lang) : "",
        answers.strictness
          ? (LABEL_MAP[answers.strictness] ?? answers.strictness)
          : "",
        answers.experience
          ? (LABEL_MAP[answers.experience] ?? answers.experience)
          : "",
        ...(answers.concerns ?? []).map((concern) => LABEL_MAP[concern] ?? concern),
      ].filter(Boolean),
    [answers]
  )
